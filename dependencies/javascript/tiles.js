		...
		$.get( "https://www.googleapis.com/youtube/v3/search?part=snippet&q=" + $('#search').val() + "&key=AIzaSyDyk41ENXaIZ7uaM9YhXA7qN6afNDoSkBc&maxResults=10" ).then(
		  function(data) {
			...
			while(data.indexOf('videoId') > -1){
				...
				document.getElementById('vid' + counter).innerHTML =  "<span title='" + title + "'>" + shortTitle + "</span><i class='fa-save fa-2x fa sample' id='button" + counter + 
				"' onclick=\"saveId('" + data.substr(data.indexOf('videoId') + 10, 11) + "')\"></i><i class='fa-play-circle-o fa fa-2x' style='margin-left: 10px;'></i>";
